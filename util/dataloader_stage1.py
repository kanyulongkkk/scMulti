...
def read_from_file(data_path, label_path = None, protien_path = None):
    ...
    #data_reader = np.transpose(data_reader)
    print("数据集大小,加了一行矩阵转秩代码:", data_reader.shape)
    protein_reader = None
    if label_path is not None:        
        label_path = os.path.join(os.path.realpath('.'), label_path)    
        labels = load_labels(label_path)
        print("标签数组大小:", labels.shape)
...
class Dataloader(data.Dataset):
    def __init__(self, train = True, data_reader = None, labels = None, protein_reader = None):
        ...
        assert self.sample_num == len(self.labels), "数据集大小与标签数组大小不匹配"
        if self.input_size_protein is not None:
            assert self.sample_num == self.protein_reader.shape[0], "数据集大小与蛋白质阵列大小不匹配"
