...
from util.closs_mmd import L1regularization, CellLoss, EncodingLoss, CenterLoss
...
class TrainingProcessStage3():
    def __init__(self, config):
        ...
        self.optimizer_encoder = optim.SGD(self.model_encoder.parameters(), lr=self.config.learning_stage3, momentum=self.config.momentum,
                                           weight_decay=0)
        self.optimizer_cell = optim.SGD(self.model_cell.parameters(), lr=self.config.learning_stage3, momentum=self.config.momentum,
                                        weight_decay=0)
    ...
    def adjust_learning_rate(self, optimizer, epoch):
        lr = self.config.learning_stage3 * (0.1 ** ((epoch - 0) // self.config.decay_epoch))
    ...
    def train(self, epoch):
        ...
        for batch_idx in range(self.training_iters):
            ...
            for iter_atac_loader in iter_atac_loaders:
                ...
                edge_index = self.calculate_edgelist(atac_embedding, atac_embedding)
                atac_cell_prediction = self.model_cell(atac_embedding,edge_index)
